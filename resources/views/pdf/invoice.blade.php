...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .invoice-number { font-size: 18px; font-weight: bold; color: #1f2937; }
        .address-block { background: #f9fafb; padding: 15px; margin-bottom: 10px; }
        .address-title { font-weight: bold; color: #6b7280; font-size: 10px; text-transform: uppercase; margin-bottom: 8px; }
        ...
        .total-row { font-size: 16px; font-weight: bold; border-top: 2px solid #1f2937; }
            ...
            <td style="width: 50%; vertical-align: top; padding-left: 10px;">
                <div class="address-block">
                    ...
                    @elseif($order->deliveryAddress)
                        <p><strong>{{ $order->deliveryAddress->prenom_adresse }} {{ $order->deliveryAddress->nom_adresse }}</strong></p>
        ...
        <tbody>
            ...
            @foreach($order->items as $item)
                @php
                    ...
                    $priceHT = $item->prix_unit_ligne / 1.20;
                ...
                <tr>
                    <td>
                        {{ $name }}
                        @if($item->size)
                            <br><small style="color: #6b7280;">Taille : {{ $item->size->nom_taille ?? $item->size->libelle_taille }}</small>
                        @endif
                    </td>
