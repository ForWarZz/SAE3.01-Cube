                ...
                <nav class="flex-1 space-y-2 px-2 py-4">
                    @if ($isCommercial)
                        ...
                        @if ($isDirector)
                            <a
                                ...
                                class="flex items-center rounded-md px-4 py-2 text-gray-300 hover:bg-slate-700 hover:text-white"
                            >
                                <x-heroicon-o-swatch class="mr-3 size-6" />
                                Statistiques
                            </a>
                        @endif
                    @elseif ($isTechnical)
                    ...
                    @else
                        <a
                            ...
                            class="flex items-center rounded-md px-4 py-2 text-gray-300 hover:bg-slate-700 hover:text-white"
                        >
                            <x-heroicon-o-shield-check class="mr-3 h-6 w-6" />
                            Tableau de bord DPO
                        </a>
                    @endif

                    <form method="POST" action="{{ route("staff.logout") }}" class="mt-auto">
                        @csrf
                        <button
                            type="submit"
            ...
            <div class="flex flex-1 flex-col overflow-hidden">
                <header class="flex items-center justify-between border-b-2 border-gray-200 bg-white px-6 py-4">
                    ...
                    <div class="flex items-center">
                        <span class="mr-2 text-gray-600">Bonjour, {{ Auth::guard("staff")->user()->prenom_staff }}</span>
                    </div>
                </header>
